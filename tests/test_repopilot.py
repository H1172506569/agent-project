...
def test_list_files_hides_internal_agent_state(tmp_path):
    ...
    assert ".repopilot" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "repopilot" / "agent" / "welcome" / "screen"
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "repopilot/0.1"
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"REPOPILOT_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "repopilot.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("repopilot.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "REPOPILOT_PROVIDER=openai",
                "REPOPILOT_OPENAI_API_BASE=https://www.right.codes/codex/v1",
                "REPOPILOT_OPENAI_API_KEY=sk-project-openai",
                "REPOPILOT_OPENAI_MODEL=gpt-5.4",
                "REPOPILOT_DEEPSEEK_API_KEY=sk-project-deepseek",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "repopilot.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = repopilot_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "repopilot.cli.OpenAICompatibleModelClient",
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".repopilot" / "runs"
...
def test_trace_and_report_redact_secret_env_values(tmp_path):
    ...
    command = subprocess.list2cmdline(
        [sys.executable, "-c", "import sys; sys.stdout.write('sk-test-secret-123')"]
    )
...
def test_final_answer_memory_labels_do_not_directly_promote_durable_memory(tmp_path):
    ...
    answer = agent.ask("Capture these stable facts into durable memory.")

    assert answer == "Project convention: Use constrained tools instead of guessing."
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".repopilot" / "memory" / "topics" / "project-conventions.md"

    assert not conventions_path.exists()
    assert "durable_promotions" not in report
    assert "durable_rejections" not in report
    assert report["memory_promotion_metrics"]["candidate_count"] == 0


def test_user_preference_promotes_through_memory_candidates(tmp_path):
    agent = build_agent(tmp_path, ["<final>Done.</final>"])

    agent.ask("Always use pytest for project verification.")

    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    preferences_path = tmp_path / ".repopilot" / "memory" / "topics" / "user-preferences.md"

    assert preferences_path.exists()
    assert "Always use pytest for project verification." in preferences_path.read_text(encoding="utf-8")
    assert report["memory_promotion_metrics"]["promoted_count"] == 1
    assert report["memory_promotion_decisions"][0]["action"] == "promote"
...
def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "repopilot", "--help"],
        capture_output=True,
        text=True,
    )
