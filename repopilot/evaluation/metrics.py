...
def build_stress_agent_metrics():
    with tempfile.TemporaryDirectory(prefix="repopilot-metrics-") as temp_dir:
        ...
        agent.memory.promote_durable(
            [("dependency-facts", f"stress-note-{index}-recall-" + ("A" * 180)) for index in range(12)]
        )
...
def _set_irrelevant_memory(agent):
    ...
    agent.memory.promote_durable([("dependency-facts", "team mascot is blue")])
...
def _set_irrelevant_memory_for_task(agent):
    ...
    state["file_summaries"] = {}
    agent.memory.state = state
    agent.memory.promote_durable([("dependency-facts", "the team mascot is blue")])
    agent.session["memory"] = agent.memory.to_dict()
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="repopilot-context-matrix-") as temp_dir:
                        ...
                        agent.memory.promote_durable(
                            [
                                ("dependency-facts", f"matrix-note-{index}-recall-" + ("A" * 180))
...
def _provider_profile(provider):
    ...
    if provider == "gpt":
        api_key = provider_env(
            ...
            ("OPENAI_API_KEY", "REPOPILOT_RIGHT_CODES_API_KEY", "RIGHT_CODES_API_KEY", "REPOPILOT_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY"),
        )
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "REPOPILOT_OPENAI_API_KEY, OPENAI_API_KEY, or shared right.codes key missing"}
        return {
            "provider": provider,
            "status": "ready",
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="repopilot-real-context-") as temp_dir:
                            ...
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            # decoy 文本带上序号，避免 durable 层按同主语互相覆盖，
                            # 否则 N 条 decoy 会塌缩成 1 条，填不满预算。
                            agent.memory.promote_durable(
                                [
                                    (
                                        "dependency-facts",
                                        ...
                                        if index == 0
                                        else f"decoy token {index} is DECOY-{index}",
                                    )
                                    for index in range(note_count)
                                ]
                            )
